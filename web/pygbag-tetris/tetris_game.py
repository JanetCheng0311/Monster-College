...
PER_LEVEL_DEC = 25
LOCK_DELAY_MS = 700
# Prefer a local font copy (packaged with the web prototype). Fall back to repo font path, else None.
FONT_PATH = os.path.join(os.path.dirname(__file__), "PixelifySans-VariableFont_wght.ttf")
...
class Tetris:
    def __init__(self, screen=None):
        ...
        if FONT_PATH is not None and os.path.exists(FONT_PATH):
            try:
                ...
                self.font = pygame.font.SysFont(None, small_fs)
                self.big_font = pygame.font.SysFont(None, big_fs)
        else:
            self.font = pygame.font.SysFont(None, small_fs)
            self.big_font = pygame.font.SysFont(None, big_fs)
    ...
    def play_intro_video(self, path):
        # Skip heavy MoviePy/OpenCV playback when not available (e.g., web build).
        if VideoFileClip is None:
            print("Intro video playback skipped: MoviePy not available in this build.")
            return None
